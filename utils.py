...
def get_interaction_res(coords_batch, pdb, top_k, 
                        threshold=5, 
                        remove_far=False, 
                        prot_only=False,
                        force_from_back=False,
                        force_from_front=False
                       ):
    chain_len_dicts = {}
    chains, lens = torch.unique_consecutive(coords_batch['chain_lens'][0][0], return_counts=True)
    chain_len_dicts['protein'] = lens[0] # first chain = "protein"
    chain_len_dicts['peptide'] = lens[1] # second chain = "peptide"
    from_back = torch.argmin(lens) == 1 # looks for shorter chain
    top_k = min(top_k, coords_batch['coords'][0].size(1))
    D_neighbors, E_idx = extract_knn(coords_batch['coords'][0][:,:,1,:], eps=1e-6, top_k=top_k) 
    if force_from_back or (from_back and not force_from_front): # second chain is masked
        interaction_res = set(range(chain_len_dicts['protein'], chain_len_dicts['protein']+chain_len_dicts['peptide']))
    else: # first chain is masked
        interaction_res = set(range(0, chain_len_dicts['protein']))
...
def get_text_and_image_features(model, tokenizer, batch, 
    ...
    if focus:
        interaction_res = get_interaction_res(coords_batch, pdb, top_k, 
...
def compute_score(batch, output_dict, weight_dists, MAX_LEN, 
                  pep_weight=1, plot_scores=None, plot_weights=None, 
                  plot_pep_mask=None, plot_indices=None, plot_X=None, 
                  plot_seq=None, is_complex=False
                 ):
